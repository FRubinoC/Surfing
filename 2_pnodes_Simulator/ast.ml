...
let rec output_management (outputs : term) (src : int) (dest : node_number) 
													(node_store : store) (channels) (functions : fun_env) : bool =
	match dest with
	| NoNode -> true
	| NodeNumber(des) ->
			...
			begin
				Chan.send target_channel values;
				true
			end
	| ParallelNodeNumber(num1, num2) ->
			(output_management outputs src num1 node_store channels functions) &&
			(output_management outputs src num2 node_store channels functions)
...
let input_management (src_node : int) (des_node : int) (inputs : string list) (node_store : store) (constants : string list) (channels) : store =
	match constants with
	...
	| _  -> let value_list = take_channel_inputs src_node des_node constants channels in
					{
						variables = insert_values inputs value_list node_store.variables;
...
let rec eval_process (nnode : int) (node_store : store) (channels) (functions : fun_env) (ast : process) : store =
	match ast with
	| InactProcess -> node_store
	| MultiOutput(outputs, dest, res_proc) ->
			let result = output_management outputs nnode dest node_store channels functions in
			if (result) then eval_process nnode node_store channels functions res_proc
	...
	| InputProc(src_node, constants, input_vars, res_proc) ->
			...
			let new_store = input_management src_node nnode input_var_lst node_store constants_lst channels in
			eval_process nnode new_store channels functions res_proc
	| ConditionProc(cond, proca, procb) ->
			let x = eval_condition node_store cond functions in
			if (x = true) then eval_process nnode node_store channels functions proca
			else eval_process nnode node_store channels functions procb
	| Assignment(x, y, res_proc) ->
			let y1 = eval_term node_store functions y in
			let new_store =
				{
					variables = update_storage x y1 node_store.variables;
					sensors   = node_store.sensors;
					actuators = node_store.actuators;
				} in
			eval_process nnode new_store channels functions res_proc
			...
			let new_store =
				{
					...
					sensors   = node_store.sensors;
					actuators = update_act_action actloc action node_store.actuators;
				} in
			eval_process nnode new_store channels functions res_proc
	| PCloseIter -> node_store
	| POpenIter(res_proc) ->
			eval_process nnode node_store channels functions res_proc


let rec eval_component (nnode : int) (node_store : store) (channels) (functions : fun_env) (ast : component) : store =
	match ast with 
	| InactiveComponent -> node_store
	| ParallelComponent(compa, compb) ->
			let new_store = eval_component nnode node_store channels functions compa in
			eval_component nnode new_store channels functions compb
	| Process(name, proc) ->
			eval_process nnode node_store channels functions proc
	| Sensor(loc, sens) ->
			eval_sensor loc node_store sens
	| Actuator(loc, act) -> node_store


let rec node_processing (nnode : int) (node_store : store) (channels) (functions : fun_env) (ast : component) =
	let new_node_store = eval_component nnode node_store channels functions ast in
	begin
		pprint_node_status nnode new_node_store channels;
		node_processing nnode new_node_store channels functions ast
	end


let rec eval_node_definition (env : iot_env) (ast : node) (channels): iot_env = 
	match ast with
	| ParallelNodes(nodea, nodeb) ->
	...
	| Node(nnode, comp) ->
		begin
			...
			| (nodes_env, fun_env) -> 
					...
					node_processing nnode node_store node_ch functions comp
		end
	| InactNode -> env

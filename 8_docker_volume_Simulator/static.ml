...
type state = Hot | Cold
type io_op = int * ((op * int * string) list) * state
...
let rec build_deadlock_outputs (const : string) (nums : Ast.node_number) : (op * int * string) list =
  match nums with
  | NoNode -> []
  | ParallelNodeNumber(num1, num2) ->
      (build_deadlock_outputs const num1) @ (build_deadlock_outputs const num2)
  | NodeNumber(num) -> (Output, num, const) :: [] 


let rec take_process_io_list (ast : Ast.process) : (op * int * string) list =
  match ast with
  | InactProcess -> []
  | PCloseIter -> []
  ...
  | ConditionProc(_, proca, procb, res_proc) -> (take_process_io_list proca)@(take_process_io_list res_proc)
  | InputProc(src,const,_,proc) -> (Input, src, const)::(take_process_io_list proc)
  | MultiOutput(const, _, nums, proc) -> (build_deadlock_outputs const nums) @ (take_process_io_list proc) 


let rec take_io_list (ast : Ast.component) : (op * int * string) list =
  match ast with
...
let rec take_first_op (node_env : io_op) : (op * int * string) =
  match node_env with
  | (num, ops, state) ->
    begin
      match ops with
      | (op, dst, const)::res_ops -> (op, dst, const)
      | [] -> raise (ErrDeadNodeOp(num))
    end


let rec remove_node_op (ops : (op * int * string) list) : (op * int * string) list =
  match ops with
  | (op, node2, const)::res_ops -> res_ops
...
let rec take_new_node (env : nodes_io) : (int * ((op * int * string) list) * state * nodes_io) =
  match env with
  | (node, ops, state)::res_env -> (node, ops, state, res_env) 
  | [] -> failwith("Deadlock checking: the environment of the nodes is empty")  


let rec check_deadlock_ops (nnode : int) (ops : (op * int * string) list) (state : state) (env : nodes_io) : bool =
  match ops with
  | (op, dst, const)::res_ops ->
      begin
        let dst_env = take_node_io dst env in
        begin
          let (dst_frst_op, dst_op_dst, dst_op_const) = take_first_op dst_env in
          match op with
          | Input -> 
              if (dst_frst_op = Output) && (dst_op_dst = nnode) && (dst_op_const = const) then
          ...
          | Output ->
              if (dst_frst_op = Input) && (dst_op_dst = nnode) && (dst_op_const = const) then
                let new_env = remove_frst_op dst env in
                check_deadlock_ops nnode res_ops state new_env
              else 
                if ((read_state dst_env) = Hot) then raise (ErrDeadlock(nnode, op, dst))
...
(* The four int are: #prepare, #release, #wait and #used , branch is 0 if it is not a branch, 1 otherwise*)
let rec check_ipc_proc (nnode : int) (pr_name : string) (vars : (string * int * int * int * int) list) (ast : Ast.process) (branch : int) : (string * int * int * int * int) list =
  match ast with
  | InactProcess ->
      if (branch = 1) then vars
      else begin
  ...
  | Prepare(var, proc) -> 
      ...
      if (var_wait != 0) || (var_rel != 0) || (var_prep != 0) || (var_used != 0) then raise (ErrPrepare(nnode, pr_name))
        ...
        check_ipc_proc nnode pr_name new_vars proc branch
      end
  | Release(var, proc) -> 
      let (var_prep, var_rel, var_wait, var_used) = take_var_info var vars in
  ...
  | POpenIter(proc) -> check_ipc_proc nnode pr_name vars proc branch
  | Assignment(_,_,proc) -> check_ipc_proc nnode pr_name vars proc branch
  | ActivateActuator(_,_,proc) -> check_ipc_proc nnode pr_name vars proc branch
  | ConditionProc(_, proca, procb, res_proc) -> 
    begin
      let checked_vars1 = check_ipc_proc nnode pr_name vars proca 1 in
      let checked_vars2 = check_ipc_proc nnode pr_name vars procb 1 in
      let new_vars =
        begin
          if (check_equal_info checked_vars1 checked_vars2) then checked_vars1
          else raise (ErrDiffSyncBranches(nnode, pr_name))
        end in
      check_ipc_proc nnode pr_name new_vars res_proc branch
    end
  | InputProc(_,_,_,proc) -> check_ipc_proc nnode pr_name vars proc branch
  | MultiOutput(_,_,_, proc) -> check_ipc_proc nnode pr_name vars proc branch
  | Encrypt(_, proc) -> check_ipc_proc nnode pr_name vars proc branch
  | Decrypt(_, proc) -> check_ipc_proc nnode pr_name vars proc branch
...
let rec check_ipc_component (nnode : int) (vars : (string * int * int * int * int) list) (ast : Ast.component) : (string * int * int * int * int) list =
  ...
  | Actuator(_,_) -> vars
  | Process(name, proc) ->
      check_ipc_proc nnode name vars proc 0

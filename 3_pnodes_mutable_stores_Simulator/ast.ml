...
let rec output_management (outputs : term) (src : int) (dest : node_number) (node_store : store) (channels) (functions : fun_env) : bool =
	match dest with
	| NoNode -> true
	| NodeNumber(des) ->
...
let input_management (src_node : int) (des_node : int) (inputs : string list) (node_store : store) (constants : string list) (channels) : unit =
	match constants with
	...
	(* //REVIEW: changing of assignment on mutable fields of the store *)
					node_store.variables <- (insert_values inputs value_list node_store.variables)
					(*
					{
...
let rec eval_process (nnode : int) (node_store : store) (channels) (functions : fun_env) (ast : process) : unit =
	match ast with
	| InactProcess -> ()
	...
	| InputProc(src_node, constants, input_vars, res_proc) ->
			...
			let _ = input_management src_node nnode input_var_lst node_store constants_lst channels in
...
let rec eval_component (nnode : int) (node_store : store) (channels) (functions : fun_env) (ast : component) : unit =
	...
	| InactiveComponent -> ()
	| ParallelComponent(compa, compb) ->
			let _ = eval_component nnode node_store channels functions compa in

...
let rec pprint_output (name : string) (src : int) (values : (value * encryption) list) (des : int) =
	begin
		let path = "./IoT-System/nodes/" ^ (string_of_int src) ^ "/" ^ name ^ ".txt" in
		let oc = open_out_gen [Open_append; Open_creat] 0o666 path in
		let _ = Printf.fprintf oc "%s" ("Output : " ^ (pprint_encr_value_list values) ^ " -> " ^ (string_of_int des) ^ "\n") in
		close_out oc
	end
...
let rec check_input (values : (value * encryption) list) (constants : string list) : (bool * ((value * encryption) list))= 
...
let rec take_channel_inputs (src : int) (dst : int) (constants : string list) (channels) : (value * encryption) list =
...
let rec pprint_input (name : string) (nnode : int) (variables : string list) (old_values : value list) (new_values : (value * encryption) list) (source : int) =
	begin	
		let path = "./IoT-System/nodes/" ^ (string_of_int nnode) ^ "/" ^ name ^ ".txt" in
		let oc = open_out_gen [Open_append; Open_creat] 0o666 path in
		let _ = Printf.fprintf oc "%s" ("Input : " ^ (pprint_string_list variables) ^ " : " ^ (pprint_value_list old_values) ^ " -> " ^ (pprint_encr_value_list new_values) ^ " : " ^ (string_of_int source) ^ "\n") in
		close_out oc
	end

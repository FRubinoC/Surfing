...
%left COMMA PARALNODE PCOMPONENT MUL DIV ADD SUB
...
condition:
	  BOOL
	  	{ Bool($1) }
	| LPAREN term CLOSEACTOUTPUT term RPAREN
		{ Greater($2, $4)}
	| LPAREN term OPENACTOUTPUT term RPAREN
		{ Lower($2, $4) }
	| LPAREN term DOUBLEEQUAL term RPAREN
		{ Equal($2, $4) }
	| LPAREN term EQUALORGREATER term RPAREN
		{ EqGr($2, $4) }
	| LPAREN term EQUALORLOWER term RPAREN
		{ EqLw($2, $4) }
;
process:
	  INACTTOKEN								/* inactive process */
		{ InactProcess }
	| OPENOUTPUT term CLOSEOUTPUT node_number DOT process  			/* output statement */
		{ MultiOutput($2, $4, $6) }
	| LPAREN INT SEMICOLON term SEMICOLON term RPAREN DOT process  		/*  input statement */
		{ InputProc($2, $4, $6, $9) }	
	| condition QUESTION process COLON process 				/* conditional statement */
		{ ConditionProc($1, $3, $5) }

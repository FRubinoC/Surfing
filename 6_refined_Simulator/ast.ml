...
let rec check_channel_existance (src : int) (const : string) (des : int) (chan_env) : bool =
	match chan_env with 
	| [] -> false 
	| (ch_src, ch_des, ch_const, _)::res_chan_env -> 
	...
	| NodeNumber(des_node) ->
			if (check_channel_existance nnode const des_node chan_env) then chan_env
				...
				begin
					...
					(nnode, des_node, const, new_chan)::chan_env
				end
...
let rec input_channel_insertion (src : int) (const : string) (des : int) (chan_env) =
	begin
		if (check_channel_existance src const des chan_env) then chan_env
			...
			begin
				let new_chan = Chan.make_bounded 0 in
				(src, des, const, new_chan)::chan_env
			end
	end
...
let rec take_channel (src : int) (const : string) (des : int) (channels) =
	match channels with 
	| (src_node, des_node, ch_const, chan)::res_channels ->
			if ((src_node = src) && (des_node = des) && (const = ch_const)) then chan
			else take_channel src const des res_channels
	| [] -> raise (ErrNotExistentChannel(src, des))


let rec find_channels (nnode : int) (channels) =
	match channels with
	| (src, dst, const, chan)::res_chan_env ->
			if ((src = nnode) || (dst = nnode)) then 
				(src, dst, const, chan)::(find_channels nnode res_chan_env)
...
let rec output_management (name : string) (outputs : term) (src : int) (const : string) (dest : node_number) (node_store : store) (channels) (functions : fun_env) : bool =
	...
	| NoNode -> true
	| NodeNumber(des) ->
			let target_channel = take_channel src const des channels in
...
let rec take_channel_inputs (src : int) (const : string) (dst : int) (channels) : (value * encryption) list =
	let target_channel = take_channel src const dst channels in
	Chan.recv target_channel
...
let input_management (name : string) (src_node : int) (const : string) (des_node : int) (inputs : string list) (node_store : store) (channels) : unit =
	let value_list = take_channel_inputs src_node const des_node channels in
